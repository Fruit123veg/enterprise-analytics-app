...
# --- LOAD EXCEL FILE ---
# Make sure your excel file name matches this exactly, or update this string:
EXCEL_FILE_NAME = "Dummy Data.xlsx" 

@st.cache_data
def load_excel_data():
    # Caches the data in memory so the app stays fast
...
if user_query:
    ...
    # SYSTEM INSTRUCTIONS: Forcing Pandas Manipulation over df_raw
    prompt = f"""
    ...
    - If the user explicitly asks for the output in a "table format", "table", or "grid", your Python plotting code should NOT create a matplotlib chart. Instead, simply write a comment `# TABLE REQUESTED` or print logic.
    ...
    - DONUT CHARTS: If a donut chart is requested, use `plt.pie(..., wedgeprops=dict(width=0.4, edgecolor='w'))`. Place the currency-formatted text labels *outside* or *clearly along* the slices so they never overlap.
    
    - READABILITY & SPACING: 
      1. Always call `plt.tight_layout()` right before saving to prevent labels from getting cut off.
      2. For bar charts, increase the top margin of the Y-axis by 15% using `ax.set_ylim(0, max_val * 1.15)` so labels above the tallest bars are clearly readable.
    
    - Clear the figure at the start using `plt.figure()`.
    - Save the plot using: `plt.savefig("{current_chart_filename}", bbox_inches="tight")`.

    Return ONLY raw python code. Do not include any standard conversational text explanations.
    """
